...
fn test_cell_qualifies_positive() {
    ...
    assert!(
        cell_qualifies(latch_cell, "G"),
        "LATCH_CELL should qualify with clock pin G"
    );
}
...
fn test_cell_qualifies_negative() {
    ...
    let latch_cell = lib.get_cell("LATCH_CELL").expect("LATCH_CELL not found");
    assert!(
        !cell_qualifies(latch_cell, "CLK"),
        "LATCH_CELL should not qualify with non-existent clock pin CLK"
    );

    let normal_cell = lib.get_cell("NORMAL_CELL").expect("NORMAL_CELL not found");
    assert!(
        !cell_qualifies(normal_cell, "G"),
        "NORMAL_CELL should not qualify (no latch group)"
    );
}
...
fn test_is_output_pin() {
    ...
    assert!(
        !is_output_pin(d_pin),
        "D should not be detected as output pin"
    );

    let g_pin = latch_cell.get_pin("G").expect("G pin not found");
    assert!(
        !is_output_pin(g_pin),
        "G should not be detected as output pin"
    );
}
...
fn test_is_input_pin() {
    ...
    assert!(
        !is_input_pin(q_pin),
        "Q should not be detected as input pin"
    );
}
...
fn test_process_library_latch_mode() {
    ...
    let q_pin = latch_cell.get_pin("Q").expect("Q pin not found");
    let clock_timing = q_pin.iter_subgroups_of_type("timing").find(|t| {
    ...
    assert!(
        ...
        "Clock timing arc should be added to Q pin"
    );
    ...
    if let Some(timing) = clock_timing {
        ...
        assert!(timing
            .iter_subgroups_of_type("rise_transition")
            .next()
            .is_some());
        assert!(timing
            .iter_subgroups_of_type("fall_transition")
            .next()
            .is_some());
    ...
    assert!(
        latch_group.is_some(),
        "Latch group should remain in latch mode"
    );
}
...
fn test_process_library_ff_mode() {
    ...
    assert!(
        ff_group.is_some(),
        "Latch group should be converted to ff in FF mode"
    );

    if let Some(ff) = ff_group {
        // Check that enable -> clocked_on and data_in -> next_state
        assert!(
            ff.simple_attribute("clocked_on").is_some(),
            "clocked_on should be set"
        );
        assert!(
            ff.simple_attribute("next_state").is_some(),
            "next_state should be set"
        );
        assert!(
    ...
    let q_pin = latch_cell.get_pin("Q").expect("Q pin not found");
    let original_timing_count = q_pin
    ...
    assert_eq!(
        original_timing_count, 0,
        "Non-clock timing arcs should be removed in FF mode"
    );
}
...
fn test_reset_pin_handling() {
    ...
    let setup_timing = rst_pin.iter_subgroups_of_type("timing").find(|t| {
        t.simple_attribute("timing_type")
            .map(|tt| tt.expr() == "setup_rising")
            .unwrap_or(false)
    });
    ...
    assert!(
        rst_pin.simple_attribute("nextstate_type").is_none(),
...
fn test_lut_template_generation() {
    ...
    let pseudo_constraint_template = lib
        .iter_subgroups_of_type("lu_table_template")
        .find(|t| t.name.contains("pseudo_constraint"));
    assert!(
        pseudo_constraint_template.is_some(),
...
"#;
    ...
    let q_clock_timing = q_pin.iter_subgroups_of_type("timing").find(|t| {
        t.simple_attribute("related_pin")
            .map(|rp| {
                rp.string() == "G"
    ...
    let qn_clock_timing = qn_pin.iter_subgroups_of_type("timing").find(|t| {
        t.simple_attribute("related_pin")
            .map(|rp| {
                rp.string() == "G"
                    && t.simple_attribute("timing_type").unwrap().expr() == "rising_edge"
    ...
    assert!(
        q_clock_timing.is_some(),
        "Q pin should have clock timing arc"
    );
    assert!(
        qn_clock_timing.is_some(),
        "QN pin should have clock timing arc"
    );
}
...
"#;
        ...
        if binary_path.exists() {
            ...
            eprintln!(
                "Content preview: {}",
                &output_content[..std::cmp::min(300, output_content.len())]
            );
            ...
            if output_content.contains("ff(IQ)") {
                assert!(
                    output_content.contains("clocked_on"),
                    "Should contain clocked_on"
                );
                assert!(
                    output_content.contains("next_state"),
                    "Should contain next_state"
                );
            } else {
